 ...
 *    + Refactored FieldLexicalVariableVR method getNamesInScope to have same named function that works on any block,
 ...
 *    + Created Blockly.LexicalVariableVR.stringListsEqual for testing equality of string lists.
 ...
 * @constructor
 */
Blockly.FieldLexicalVariableVR = function(varname) {
 // Call parent's constructor.
  Blockly.FieldDropdown.call(this, Blockly.FieldLexicalVariableVR.dropdownCreate,
                                   Blockly.FieldLexicalVariableVR.dropdownChange);
  if (varname) {
...
// FieldLexicalVariableVR is a subclass of FieldDropdown.
goog.inherits(Blockly.FieldLexicalVariableVR, Blockly.FieldDropdown);

/**
 * Set the variable name.
 * @param {string} text New text.
 */
Blockly.FieldLexicalVariableVR.prototype.setValue = function(text) {
  Blockly.FieldLexicalVariableVR.superClass_.setValue.call(this, text);
  this.updateMutation();
};
 ...
 */
Blockly.FieldLexicalVariableVR.prototype.updateMutation = function() {
 ...
 */
Blockly.FieldLexicalVariableVR.prototype.getBlock = function() {
  return this.block_; 
};
 ...
 */
Blockly.FieldLexicalVariableVR.prototype.setBlock = function(block) {
  this.block_ = block;
 ...
 */
Blockly.FieldLexicalVariableVR.prototype.getCachedParent = function() {
 ...
 */
Blockly.FieldLexicalVariableVR.prototype.setCachedParent = function(parent) {
...
Blockly.FieldLexicalVariableVR.getGlobalNames = function (optExcludedBlock) {
  ...
  if (Blockly.mainWorkspace) {
    ...
    for (var i = 0; i < blocks.length; i++) {
      var block = blocks[i];
      if ((block.type === 'vr_global_declaration') && (block != optExcludedBlock)) {
...
/**
 * @this A FieldLexicalVariableVR instance
 * @returns {list} A list of all global and lexical names in scope at the point of the getter/setter
 *   block containing this FieldLexicalVariableVR instance. Global names are listed in sorted
...
Blockly.FieldLexicalVariableVR.prototype.getNamesInScope = function () {
  return Blockly.FieldLexicalVariableVR.getNamesInScope(this.block_);
}
...
Blockly.FieldLexicalVariableVR.getNamesInScope = function (block) {
  var globalNames = Blockly.FieldLexicalVariableVR.getGlobalNames(); // from global variable declarations
  // [lyn, 11/24/12] Sort and remove duplicates from namespaces
  globalNames = Blockly.LexicalVariableVR.sortAndRemoveDuplicates(globalNames);
  var allLexicalNames = Blockly.FieldLexicalVariableVR.getLexicalNamesInScope(block);
...
Blockly.FieldLexicalVariableVR.getLexicalNamesInScope = function (block) {
  ...
  if (child) {
    ...
    if (parent) {
      while (parent) {
          ...
          } else if ( (parent.type === "vr_controls_forEach")
  ...
  if(!Blockly.usePrefixInYail){ // Only a single namespace
    allLexicalNames = procedureParamNames.concat(handlerParamNames)
                                         ...
                                         .concat(rangeNames)
                                         .concat(localNames);
    allLexicalNames = Blockly.LexicalVariableVR.sortAndRemoveDuplicates(allLexicalNames);
 ...
 * @this {!Blockly.FieldLexicalVariableVR}
 */
Blockly.FieldLexicalVariableVR.dropdownCreate = function() {
 ...
 * @this {!Blockly.FieldLexicalVariableVR}
 */
Blockly.FieldLexicalVariableVR.dropdownChange = function(text) {
  if (text) {
    this.setValue(text);
 ...
 */
Blockly.FieldLexicalVariableVR.nameNotIn = function(name, nameList) {
  ...
  // suffixes is ["17", "5"]
  var namePrefixSuffix = Blockly.FieldLexicalVariableVR.prefixSuffix(name);
  ...
  for (var i = 0; i < nameList.length; i++) {
    var prefixSuffix = Blockly.FieldLexicalVariableVR.prefixSuffix(nameList[i]);
 ...
 */
Blockly.FieldLexicalVariableVR.prefixSuffix = function(name) {
  var prefix = name;
  var suffix = "";
...
Blockly.LexicalVariableVR.renameGlobal = function (newName) {

  // this is bound to field_textinput object 
  var oldName = this.text_;

  // [lyn, 10/27/13] now check legality of identifiers
  newName = Blockly.LexicalVariableVR.makeLegalIdentifier(newName);

  var globals = Blockly.FieldLexicalVariableVR.getGlobalNames(this.sourceBlock_); 
    // this.sourceBlock excludes block being renamed from consideration
  // Potentially rename declaration against other occurrences
  newName = Blockly.FieldLexicalVariableVR.nameNotIn(newName, globals);
...
Blockly.LexicalVariableVR.renameParam = function (newName) {
  ...
  return Blockly.LexicalVariableVR.renameParamFromTo(this.sourceBlock_, oldName, newName, false);
  // Default should be false (as above), but can also play with true:
  // return Blockly.LexicalVariableVR.renameParamFromTo(this.sourceBlock_, oldName, newName, true);
}
 ...
 *   of this name).
 */
Blockly.LexicalVariableVR.renameParamFromTo = function (block, oldName, newName, renameCapturables) {
  if (block.type && block.type.indexOf("mutator") != -1) { // Handle mutator blocks specially
    return Blockly.LexicalVariableVR.renameParamWithoutRenamingCapturables(block, oldName, newName, []);
  } else if (renameCapturables) {
    Blockly.LexicalVariableVR.renameParamRenamingCapturables(block, oldName, newName);
    return newName;
  } else {
    return Blockly.LexicalVariableVR.renameParamWithoutRenamingCapturables(block, oldName, newName, []);
  }
 ...
 * @param newName
 */
Blockly.LexicalVariableVR.renameParamRenamingCapturables = function (sourceBlock, oldName, newName) {
  if (newName !== oldName) { // Do nothing if names are the same
    ...
    if (namesDeclaredHere.indexOf(oldName) == -1) {
      throw "Blockly.LexicalVariableVR.renamingCapturables: oldName " + oldName +
          " is not in declarations {" + namesDeclaredHere.join(',') + "}";
    }
    var namesDeclaredAbove = Blockly.FieldLexicalVariableVR.getNamesInScope(sourceBlock);
    ...
    if (declaredNames.indexOf(newName) != -1) {
      throw "Blockly.LexicalVariableVR.renameParamRenamingCapturables: newName " + newName +
      ...
      if (sourceBlock.renameBound) {
        ...
        throw "Blockly.LexicalVariableVR.renameParamRenamingCapturables: block " + sourceBlock.type +
 ...
 */
Blockly.LexicalVariableVR.renameFree = function (block, freeSubstitution) {
  if (block) { // If block is falsey, do nothing.
    if (block.renameFree) {  // should be defined on every declaration block
      ...
      block.getChildren().map( function(blk) { Blockly.LexicalVariableVR.renameFree(blk, freeSubstitution); } );
    }
  }
}
 ...
 */
Blockly.LexicalVariableVR.freeVariables = function (block) {
  var result = [];
  if (!block) { // input and next block slots might not empty
    ...
    var nameSets = block.getChildren().map( function(blk) { return Blockly.LexicalVariableVR.freeVariables(blk); } );
 ...
 *   of this name).
 */
Blockly.LexicalVariableVR.renameParamWithoutRenamingCapturables = function (sourceBlock, oldName, newName, OKNewNames) {
  if (oldName === newName) {
    return oldName;
  }
  var namesDeclaredHere = sourceBlock.declaredNames ? sourceBlock.declaredNames() : [];
...
//  }
  ...
  if (Blockly.showPrefixToUser) {
    ...
    } else if (sourceBlock.type == "vr_controls_forEach") {
      sourcePrefix = Blockly.loopParameterPrefix;
    } else if ( sourceBlock.type == "vr_controls_forRange") {
      sourcePrefix = Blockly.loopRangeParameterPrefix;
    } else if (sourceBlock.type == "vr_local_declaration_statement"
               || sourceBlock.type == "vr_local_declaration_expression"
               || sourceBlock.type == "vr_local_mutatorarg") {
      sourcePrefix = Blockly.localNamePrefix;
    }
  }
  var helperInfo = Blockly.LexicalVariableVR.renameParamWithoutRenamingCapturablesInfo(sourceBlock, oldName, sourcePrefix);
  ...
  var conflicts = Blockly.LexicalVariableVR.sortAndRemoveDuplicates(capturables.concat(declaredNames));
  newName = Blockly.FieldLexicalVariableVR.nameNotIn(newName, conflicts);
 ...
 */
Blockly.LexicalVariableVR.renameParamWithoutRenamingCapturablesInfo = function (sourceBlock, oldName, sourcePrefix) {
  ...
  var referenceResults = inScopeBlocks.map( function(blk) { return Blockly.LexicalVariableVR.referenceResult(blk, oldName, sourcePrefix, []); } );
  ...
  capturables = Blockly.LexicalVariableVR.sortAndRemoveDuplicates(capturables);
 ...
 */
Blockly.LexicalVariableVR.checkIdentifier = function(ident) {
...
Blockly.LexicalVariableVR.makeLegalIdentifier = function(ident) {
  var check = Blockly.LexicalVariableVR.checkIdentifier(ident);
  if (check.isLegal) {
...
Blockly.LexicalVariableVR.referenceResult = function (block, name, prefix, env) {
  ...
  if (block.type === "vr_controls_forEach") {
    ...
    var listResults = Blockly.LexicalVariableVR.referenceResult(block.getInputTargetBlock('LIST'), name, prefix, env);
    var doResults = Blockly.LexicalVariableVR.referenceResult(block.getInputTargetBlock('DO'), name, prefix, newEnv);
    var nextResults = Blockly.LexicalVariableVR.referenceResult(Blockly.LexicalVariableVR.getNextTargetBlock(block), name, prefix, env);
    referenceResults = [listResults,doResults,nextResults];
  } else if (block.type === "vr_controls_forRange") {
    var loopVar = block.getFieldValue('VAR');
    if (Blockly.usePrefixInYail) { // Invariant: Blockly.showPrefixToUser must also be true!
      loopVar = (Blockly.possiblyPrefixMenuNameWith(Blockly.loopRangeParameterPrefix))(loopVar)
    }
    var newEnv = env.concat([loopVar]);
    var startResults = Blockly.LexicalVariableVR.referenceResult(block.getInputTargetBlock('START'), name, prefix, env);
    var endResults = Blockly.LexicalVariableVR.referenceResult(block.getInputTargetBlock('END'), name, prefix, env);
    var stepResults = Blockly.LexicalVariableVR.referenceResult(block.getInputTargetBlock('STEP'), name, prefix, env);
    var doResults = Blockly.LexicalVariableVR.referenceResult(block.getInputTargetBlock('DO'), name, prefix, newEnv);
    var nextResults = Blockly.LexicalVariableVR.referenceResult(Blockly.LexicalVariableVR.getNextTargetBlock(block), name, prefix, env);
    referenceResults = [startResults,endResults,stepResults,doResults,nextResults];
  } else if ((block.type === "vr_local_declaration_statement") || (block.type === "vr_local_declaration_expression")) {
    ...
    var initResults = localInits.map( function(init) { return Blockly.LexicalVariableVR.referenceResult(init, name, prefix, env); } );
    if (block.type === "vr_local_declaration_statement") {
      var doResults = Blockly.LexicalVariableVR.referenceResult(block.getInputTargetBlock('STACK'), name, prefix, newEnv);
      var nextResults = Blockly.LexicalVariableVR.referenceResult(Blockly.LexicalVariableVR.getNextTargetBlock(block), name, prefix, env);
      referenceResults = initResults.concat([doResults,nextResults]); 
    } else { // (block.type === "local_declaration_expression") {
      var returnResults = Blockly.LexicalVariableVR.referenceResult(block.getInputTargetBlock('RETURN'), name, prefix, newEnv);
      referenceResults = initResults.concat([returnResults]); 
    }
  } else { // General case for blocks that do not introduce new names
   referenceResults = block.getChildren().map( function(blk) { return Blockly.LexicalVariableVR.referenceResult(blk, name, prefix, env); } );
  }
  var blocksToRename = [];
  var capturables = [];
  for (var r = 0; r < referenceResults.length; r++) {
  ...
  if ((block.type === "vr_lexical_variable_get") || (block.type === "vr_lexical_variable_set")) {
...
Blockly.LexicalVariableVR.sortAndRemoveDuplicates = function (strings) {
...
Blockly.LexicalVariableVR.getNextTargetBlock = function (block) {
 ...
 */
Blockly.LexicalVariableVR.stringListsEqual = function (strings1, strings2) {
 ...
 */
Blockly.LexicalVariableVR.eventParamMutationToDom = function (block) {
  if (!block.eventparam) {      // Newly created block won't have one
    Blockly.LexicalVariableVR.getEventParam(block);
  }
...
Blockly.LexicalVariableVR.getEventParam = function (block) {
  ...
  if (prefix !== Blockly.globalNamePrefix) {
    ...
    while (parent) {
       ...
       } else if ( ( (parent.type === "vr_controls_forEach") || (parent.type === "vr_controls_forRange") )
                   && (parent.getInputTargetBlock('DO') == child) ) { // Only DO is in scope, not other inputs!
         var loopName = parent.getFieldValue('VAR');
           if (loopName == name) {
             return null; // Name is locally bound, not an event parameter.
 ...
 * This is used only by Blockly.LexicalVariableVR.eventParameterDict
 */
Blockly.LexicalVariableVR.eventParamDomToMutation = function (block, xmlElement) {
  ...
  if (children.length == 1) { // Should be exactly one eventParam child
    ...
    if (childNode.nodeName.toLowerCase() == 'eventparam') {
      ...
      block.eventparam = untranslatedEventName; // special property viewed by Blockly.LexicalVariableVR.eventParameterDict
    }
  }
}
 ...
 */
Blockly.LexicalVariableVR.eventParameterDict = function (block) {
  var dict = {};
